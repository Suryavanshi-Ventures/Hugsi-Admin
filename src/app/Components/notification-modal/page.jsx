...
const NotificationModal = ({
  ...
  userId,
  userAllDetails,
  sendingSelectedIdsToNotification,
}) => {
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      if (
        sendingSelectedIdsToNotification &&
        sendingSelectedIdsToNotification.length > 0
      ) {
        ...
        <div className="bg-white w-1/3 p-8 rounded-md">
          ...
          sendingSelectedIdsToNotification.length > 0 ? (
            <h2 className="text-lg font-semibold mb-4">
              ...
              <span className="text-[#FFBF00]">
                {sendingSelectedIdsToNotification.length}
              </span>{" "}
              users
            </h2>
          ) : (
          ...
          <div className="mb-4">
            ...
            <input
              type="text"
              id="title"
              className={`mt-1 p-2 border border-gray-300 focus:outline-none focus:border-[#FFBF00] rounded-md  w-full ${
          ...
          <div className="mb-4">
            ...
            <textarea
              id="body"
              rows="4"
              className={`mt-1 p-2 border rounded-md focus:outline-none focus:border-[#FFBF00] w-full ${
            ...
            />
            {bodyError && (
              <p className="text-red-500 font-semibold">{bodyError}</p>
            )}
          </div>
